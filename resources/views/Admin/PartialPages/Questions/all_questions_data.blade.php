                            ...
                            <div class="table-responsive">
                                <table id="zero_config" class="table table-striped table-bordered text-center">
                                    ...
                                    <tr>
                                        <th style="width: 1%;">
                                            @if(Permission::can('verifyDraftQuestionUpdate'))
                                                @if(count($questions) > $shareQCount)
                                                    <input type="checkbox" id="alloptionverify" name="allverify"
                                                           class="material-inputs alloptionverify">
                                                    <label for="alloptionverify"></label>
                                                @endif
                                            @else
                                                <input type="checkbox" id="alloptionverify" class="material-inputs"
                                                       disabled>
                                                <label for="alloptionverify"></label>
                                            @endif
                                        </th>
                                        ...
                                        <th style="width: 3%;"
                                            class="text-center">{{__('form.action') .'/'.__('form.status')}}</th>
                                    </tr>
                                    </thead>
                                    <tbody>
                                    @foreach($questions as $qs)
                                        <tr>
                                            <td>
                                                ...
                                                @if(Permission::can('verifyDraftQuestionUpdate'))
                                                    @if(!$qs->shareQuestion)
                                                        <input type="checkbox" id="optionveri{{$qs->id}}"
                                                               class="material-inputs verifyelement" name="verified"
                                                               value="{{$qs->id}}">
                                            ...
                                            <td class="text-center" id="fileavi_{{$qs->id}}">
                                                @if($qs->fileType == 'image' || $qs->fileType == 'video' || $qs->fileType == 'audio')
                                                    @if($qs->fileType == 'image')
                                                        <img src="{{asset($qs->question_file_link)}}" alt="" width="150"
                                                             height="100">
                                                    @elseif($qs->fileType == 'video')
                                                        <video width="150" height="100" controls>
                                                            <source src="{{asset($qs->question_file_link)}}"
                                                                    type="video/mp4">
                                                            <source src="{{asset($qs->question_file_link)}}"
                                                                    type="video/ogg">
                                            ...
                                            <td>
                                                @if($qs->question_text)
                                                    <p id="eq_{{$qs->id}}"
                                                       class="p-1 border border-success rounded-lg">{{$qs->question_text}}</p>
                                                @endif
                                                ...
                                                @if($qs->bd_question_text)
                                                    <p id="bq_{{$qs->id}}"
                                                       class="p-1 border border-primary rounded-lg">{{$qs->bd_question_text ? $qs->bd_question_text : ''}}</p>
                                                @endif
                                            </td>
                                            <td class="text-center">
                                                    ...
                                                    @foreach($qs->options as $qo)
                                                            @if($qo->option)
                                                                <span class="btn btn-sm m-1 rounded-lg"
                                                                      style="border: #5378e8 1px solid;">
                                                                @if($qo->correct)
                                                                        <i class="{{$qo->correct?'fa fa-check':''}}"
                                                                           style="color:#5378e8"></i>
                                                                    @endif
                                                                    {{$qo->option}}
                                                            </span>
                                                            @else
                                            ...
                                            <td class="text-center">
                                                ...
                                                @if(Permission::can('draftQuestion.edit'))
                                                    <a class="edit text-info" style="cursor: pointer; color:black;"
                                                       data-id="{{$qs->id}}" title="edit">
                                                ...
                                                <span id="difficulty_{{$qs->id}}">
                                                         @if($qs->difficulty)
                                                        <span
                                                            class="badge badge-pill {{$qs->difficulty->id == 1 ? 'badge-secondary' :($qs->difficulty->id == 2 ? 'badge-cyan' : 'badge-danger')}}">
