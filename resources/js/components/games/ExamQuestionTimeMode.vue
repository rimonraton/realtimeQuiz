...
<!--                        <a @click="gameResetCall" v-if="user.id == uid && qid > 0 " class="btn btn-sm btn-danger float-right">RESET</a>-->
                    ...
                    <div class="card-body">
                        <div class="d-flex justify-content-between flex-column">
                            <span> {{ user.lang == 'gb' ? 'Exam Name' : 'পরীক্ষার নাম' }} : {{ tbe(id.exam_bn,id.exam_en,user.lang) }}</span>
    ...
    export default {
        ...
        methods: {
            ...
            totalTime(){
                ...
                let hasMinutes
                let hasSeconds
                const hours = this.id.question_time > 3600 ? Math.floor(this.id.question_time/3600) : 0
                const minutes = Math.floor(this.id.question_time/60)
                const seconds = Math.floor(this.id.question_time % 60)
            ...
            perQTime(){
                let hasMinutes
                let hasSeconds
                const qTime = this.id.question_time/this.questions.length
